...
SWENumericalFluxHLLC::validParams()
{
  ...
  params.addParam<bool>("use_pvrs", true, "Use PVRS star-depth/q-scaled wave speeds (else Einfeldt)");
  params.addParam<Real>("degeneracy_eps",
                        1e-8,
                        "Small epsilon for degeneracy/positivity tests and S* clamping");
  params.addParam<Real>("blend_alpha",
                        0.0,
                        "Optional compression-based blend to HLLE (0 disables; 0.2-0.3 typical)");
                               ...
                               std::vector<Real> & flux) const
{
  ...
  Real hL = std::max(uvec1[0], 0.0), huL = (hL > _h_eps) ? uvec1[1] : 0.0, hvL = (hL > _h_eps) ? uvec1[2] : 0.0;
  Real hR = std::max(uvec2[0], 0.0), huR = (hR > _h_eps) ? uvec2[1] : 0.0, hvR = (hR > _h_eps) ? uvec2[2] : 0.0;
  ...
  if (has_b)
  {
    ...
    hL = hLstar; hR = hRstar;
    huL = uLx * hL; hvL = uLy * hL;
    huR = uRx * hR; hvR = uRy * hR;
  }
  ...
  if (_use_pvrs)
  {
    ...
    const Real qL = (h_pvrs <= hL || hL <= _h_eps) ? 1.0 : std::sqrt(1.0 + std::max(0.0, h_pvrs / hL - 1.0));
    const Real qR = (h_pvrs <= hR || hR <= _h_eps) ? 1.0 : std::sqrt(1.0 + std::max(0.0, h_pvrs / hR - 1.0));
    SL = unL - aL * qL;
    SR = unR + aR * qR;
  }
  else
  ...
  Real hLstar = hL * (SL - unL) / (std::abs(denomL) > _degeneracy_eps ? denomL : std::copysign(_degeneracy_eps, denomL));
  Real hRstar = hR * (SR - unR) / (std::abs(denomR) > _degeneracy_eps ? denomR : std::copysign(_degeneracy_eps, denomR));
  ...
  auto hlle = [&](const std::vector<Real> & A, const std::vector<Real> & B,
                  const std::vector<Real> & FA, const std::vector<Real> & FB,
                  Real SLv, Real SRv)
  {
    std::vector<Real> f(3, 0.0);
                                   ...
                                   DenseMatrix<Real> & jac2) const
{
  ...
  jac1.resize(3,3); jac2.resize(3,3); jac1.zero(); jac2.zero();
  ...
  auto fill_dF = [&](const std::vector<Real> & U, DenseMatrix<Real> & J)
  {
    ...
    if (h > _h_eps)
    {
      ...
      J(1,0) = hu*d_un_dh + _g*h*nx;  J(1,1) = un + hu*d_un_dhu;  J(1,2) = hu*d_un_dhv;
      J(2,0) = hv*d_un_dh + _g*h*ny;  J(2,1) = hv*d_un_dhu;       J(2,2) = un + hv*d_un_dhv;
    }
  };
  ...
  const Real unL = (hL>_h_eps)?(huL*nx+hvL*ny)/hL:0.0;
  const Real unR = (hR>_h_eps)?(huR*nx+hvR*ny)/hR:0.0;
  const Real smax = std::max(std::fabs(unL)+std::sqrt(_g*hL), std::fabs(unR)+std::sqrt(_g*hR));
  for (unsigned i=0;i<3;++i) { jac1(i,i)+=0.5*smax; jac2(i,i)+=0.5*smax; }
}

void SWENumericalFluxHLLC::initialize()
{
