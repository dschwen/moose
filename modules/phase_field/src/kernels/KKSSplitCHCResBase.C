...
InputParameters
KKSSplitCHCResBase::validParams()
{
...
KKSSplitCHCResBase::KKSSplitCHCResBase(const InputParameters & parameters,
                                       const std::string & ca_name)
  : DerivativeMaterialInterface<JvarMapKernelInterface<SplitCHBase>>(parameters),
    _dFadca(getMaterialPropertyDerivative<Real>("fa_name", ca_name)),
    ...
    _w(coupledValue("w"))
{
  ...
  for (unsigned int i = 0; i < _n_args; ++i)
    _d2Fadcadarg[i] = &getMaterialPropertyDerivative<Real>("fa_name", ca_name, i);
}

void
KKSSplitCHCResBase::initialSetup()
{
...
Real
KKSSplitCHCResBase::computeQpResidual()
{
 ...
 */
Real
KKSSplitCHCResBase::computeDFDC(PFFunctionType type)
{
  switch (type)
  {
    case Residual:
...
Real
KKSSplitCHCResBase::computeQpOffDiagJacobian(unsigned int jvar)
{
