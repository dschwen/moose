    ...
    _zdata(_numx * numy)
{
  ...
  Real x, y, z;
  while (stream_in iss >> x >> y >> z)
  {
    if (x >= _xmin && y >= _ymin && x < _xmax && y < _ymax)
    {
      const unsigned int ax = numx * (x - _xmin) / (_xmax - _xmin);
      const unsigned int ay = numy * (y - _ymin) / (_ymax - _ymin);
...
{
  const unsigned int ax = numx * (p(0) - _xmin) / (_xmax - _xmin);
  const unsigned int ay = numy * (p(1) - _ymin) / (_ymax - _ymin);

  return _zdata[ax + _numx * y];
}
