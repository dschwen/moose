...
CrystalPlasticitySlipRateGSS::readFileFlowRateParams()
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    for (unsigned int j = 0; j < _num_slip_sys_flowrate_props; ++j)
      if (!(file >> vec[j]))
        mooseError("Error FiniteStrainUObasedCP: Premature end of slip_sys_flow_rate_param file");
...
CrystalPlasticitySlipRateGSS::getFlowRateParams()
{
  if (_flowprops.size() <= 0)
    mooseError("FiniteStrainUObasedCP: Error in reading flow rate  properties: Specify input in .i file or a slip_sys_flow_prop_file_name");
  ...
  for (unsigned int i = 0; i < _flowprops.size()/num_data_grp; ++i)
  {
    ...
    if (is > ie)
      mooseError("FiniteStrainUObasedCP: Start index is = " << is << " should be greater than end index ie = " << ie << " in flow rate parameter read");
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    {
      mooseWarning( "FiniteStrainUObasedCP: Non-positive flow rate parameters " << _a0(i) << "," << _xm(i) );
      break;
    }
  }
}
