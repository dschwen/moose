...
ContactPressureAux::ContactPressureAux(const InputParameters & params) :
    ...
    _penetration_locator(getPenetrationLocator(getParam<BoundaryName>("paired_boundary"), getParam<std::vector<BoundaryName> >("boundary")[0], getParam<MooseEnum>("order").getEnum<Order>()))
{
}
