...
/**
 * Tenmplate class to represent a "zero" value. Having zeroness associated with a type
...
public:
  CTNull() {}
  const auto operator()() const { return T(0); }
...
/**
 * Tenmplate class to represent a "one" value. Having oneness associated with a type
 * enables compile time optimizations in some operators.
 */
template <typename T>
...
public:
  CTOne() {}
  const auto operator()() const { return T(1); }
...
public:
  ...
  const auto operator()() const { return _ref; }
...
class CTArrayRef : public CTRef<tag, T>
{
public:
  CTArrayRef(const T & ref, const I & idx) : CTRef<tag, T>(ref), _idx(idx) {}
  const auto operator()() const { return _ref[_idx]; }

  template <CTTag dtag>
  auto D() const
  {
    if constexpr (tag == dtag)
      return CTOne<typename T::value_type>();
    else
      return CTNull<typename T::value_type>();
  }

  typedef typename T::value_type O;
  using CTRef<tag, T>::_ref;
