...
template <ComputeStage compute_stage>
void
ADKernelGrad<compute_stage>::computeADOffDiagJacobian()
{
  const unsigned int n_test = _test.size();
  std::vector<DualReal> residuals(n_test, 0);
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    for (_i = 0; _i < n_test; ++_i)
      residuals[_i] += value * _grad_test[_i][_qp];
  }
...
template <>
void
ADKernelGrad<RESIDUAL>::computeADOffDiagJacobian()
{
}
